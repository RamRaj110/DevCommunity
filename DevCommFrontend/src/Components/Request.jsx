    ...
    return (
      <div className="flex flex-col justify-center items-center min-h-screen bg-slate-950 text-white">
        ...
        <p className="text-slate-400 mt-2">
          Go to the feed to connect with more developers.
        </p>
      </div>
    );
  }
      ...
      <div className="relative z-10 max-w-3xl mx-auto">
        ...
        <div className="space-y-4">
          {requestData.map((item) => {
            const { firstName, lastName, gender, about, profileImg, skills } =
              item.fromUserId;
                  ...
                  {skills?.length > 0 && (
                    <div className="flex flex-wrap justify-center sm:justify-start gap-2">
                      {skills.slice(0, 4).map((skill, i) => (
                        <span
                          key={i}
                          className="px-2 py-1 text-xs font-medium text-blue-300 bg-blue-900/30 border border-blue-500/20 rounded-md"
                        >
                          {skill}
                        </span>
                      ))}
